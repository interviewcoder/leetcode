...
/** see test {@link _200_NumberOfIslands.SolutionBFSTest } */
public class SolutionBFS {
    
    // convert 2d coordinates to 1d coordinates, which can also be achieved
    // by Java native Point class
    ...
    private void labelReachableLand(int i, int j, int rows, int cols,
        ...
        while (!landQueue.isEmpty()) {
            ...
            if (row >= 0 && row < rows && col >= 0 && col < cols
                ...
                if (col - 1 >= 0) {
                    // avoid [row, -1] be converted to [row-1, cols - 1]
                    landQueue.add(row * cols + (col - 1));
                }
                if (col + 1 < cols) {
                    // avoid [row, cols] be converted to [row + 1, 0]
                    landQueue.add(row * cols + (col + 1));
                }
            }
        }
